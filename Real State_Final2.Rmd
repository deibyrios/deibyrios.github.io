...
all_permits <- sales_upper_east %>%
  group_by(Year, Block) %>% #create variable "Year"
  summarise(Total = n()) %>%                    #total permits filed by block per year
  arrange(Year)                                 #arrange from lowest to highest year
#head(all_permits)
#write.table(all_permits,file="all_permits.csv",sep=",",row.names=F)

all_permits$Total[all_permits$Total==0] <- 1     # US$1, just to avoid division by zero
...
```{r, include=FALSE}
blocks_df_var <- blocks_df_var %>%
  select(Distance, avg.index)

blocks <- merge(blocks_df, blocks_df_var, by=c("Distance"))
colnames(blocks) <- c("Distance","Year","Block","Total","avg.index")

#Before merging, lets spread permits_var so each year is a column containing the avg.index: 
blocks_df_var <- blocks %>%
  select(-contains("Total"))
blocks_df_var <- spread(blocks_df_var, Year, avg.index)
#head(permits_var)
colnames(permits_var)[3:17] <- paste("Y", colnames(permits_var)[3:17], sep = "_")
#colnames(permits_var)
#write.table(permits_var,file="spread_permits_avg_var.csv",sep=",",row.names=F)
...
NYC_Map2 <- merge(NYC_Map2, blocks_df_var, by=c("Block"))
