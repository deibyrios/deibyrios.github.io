...
permits_var_prep <- permits_var %>%
  select(-contains("Total"), -starts_with("index")) 
permits_var_spread <- spread(permits_var_prep, Year, avg.index)
#head(permits_var)
colnames(permits_var_spread)[3:17] <- paste("Y", colnames(permits_var_spread)[3:17], sep = "_")
...
```{r, include = FALSE}
# section for real estate data

pre_subway_re_data <- read.csv("ues_data_14-16.csv", header = TRUE)
post_subway_re_data <- read.csv("data/all_ues_sales_geocode_2017-2018.csv", header = TRUE)
...
blocked_pre_subway_re <- pre_subway_re_data %>% filter(price > 0) %>% select(year, block, price) %>% group_by(year, block) %>% summarize(qty_sales=n(), notional_sales = sum(price)) %>% rename(Year = year, Block = block)

blocked_post_subway_re <- post_subway_re_data %>% filter(SALE.PRICE > 0) %>% select(YEAR, BLOCK, SALE.PRICE) %>% group_by(YEAR, BLOCK) %>% summarize(qty_sales=n(), notional_sales = sum(SALE.PRICE)) %>% rename(Year = YEAR, Block = BLOCK)

blocked_subway_re <- blocked_pre_subway_re %>% bind_rows(blocked_post_subway_re)


re_var <- blocked_subway_re %>%
  group_by(Block) %>%
  mutate(qty_index = qty_sales/qty_sales[1]-1, notional_index = notional_sales/notional_sales[1]-1) %>% ungroup()
...
#Average Var Index per Distance:
re_var[is.na(re_var)] <- 0 #replace NA's by zero
re_var <- re_var %>%
  filter(Block != 1111) %>% #this is central park!
  group_by(Year,Distance) %>%
  mutate(avg.qty_index = mean(qty_index, na.rm = TRUE), avg.notional_index = mean(notional_index, na.rm = TRUE))
...
# Spread this too
re_var_prep_qty <- re_var %>%
  select(-c(qty_sales, notional_sales, qty_index, notional_index, avg.notional_index)) 
re_var_spread_qty <- spread(re_var_prep_qty, Year, avg.qty_index)
colnames(re_var_spread_qty)[3:7] <- paste("Qty_Y", colnames(re_var_spread_qty)[3:7], sep = "_")

re_var_prep_notional <- re_var %>%
  select(-c(qty_sales, notional_sales, qty_index, notional_index, avg.qty_index)) 
re_var_spread_notional <- spread(re_var_prep_notional, Year, avg.notional_index)
colnames(re_var_spread_notional)[3:7] <- paste("Notional_Y", colnames(re_var_spread_notional)[3:7], sep = "_")

re_var_spread <- re_var_spread_qty %>% bind_cols(re_var_spread_notional) %>% select(-c(Block1, Distance1))
...
NYC_Map_Permits <- merge(NYC_Map2, permits_var_spread, by="Block")
NYC_Map_Permits@data[is.na(NYC_Map_Permits@data)] <- 0
NYC_Map_Permits@data$Distance[NYC_Map_Permits@data$Distance==0] <- "other" #replace 0 Distance with "other"
NYC_Map_Permits@data$avg.index[NYC_Map_Permits@data$Distance=="other"] <- 0
#head(NYC_Map_Permits@data)

#write.table(NYC_Map_Permits@data,file="NYC_Map_Permits_data.csv",sep=",",row.names=F)

#set color scale to be used when mapping variation index for building permits:
palette <- colorNumeric(palette = "Blues", domain = NYC_Map_Permits$Y_2015) #continuous range

#Set interactive labels for 2001-2005 Layer: 
block_labels2005 <- sprintf("<strong>%s from 2nd Ave Line:</strong> %s avg. increase<br/>in Bldg Permits (2001-2005)<br/>Zip Code: %s, Block: %s, Lot: %s<br/>Address: %s",NYC_Map_Permits$Distance,
                            ifelse(NYC_Map_Permits@data$Distance=='other',"NA", percent(NYC_Map_Permits$Y_2005)),
                            NYC_Map_Permits$ZipCode, NYC_Map_Permits$Block, NYC_Map_Permits$Lot, NYC_Map_Permits$Address) %>%
  lapply(htmltools::HTML)

#Set interactive labels for 2001-2010 Layer: 
block_labels2010 <- sprintf("<strong>%s from 2nd Ave Line:</strong> %s avg. increase<br/>in Bldg Permits (2001-2010)<br/>Zip Code: %s, Block: %s, Lot: %s<br/>Address: %s",NYC_Map_Permits$Distance,
                            ifelse(NYC_Map_Permits@data$Distance=='other',"NA", percent(NYC_Map_Permits$Y_2010)),
                            NYC_Map_Permits$ZipCode, NYC_Map_Permits$Block, NYC_Map_Permits$Lot, NYC_Map_Permits$Address) %>%
  lapply(htmltools::HTML)

#Set interactive labels for 2001-2015 Layer Shapefile: 
block_labels2015 <- sprintf("<strong>%s from 2nd Ave Line:</strong> %s avg. increase<br/>in Bldg Permits (2001-2015)<br/>Zip Code: %s, Block: %s, Lot: %s<br/>Address: %s",NYC_Map_Permits$Distance,
                            ifelse(NYC_Map_Permits@data$Distance=='other',"NA", percent(NYC_Map_Permits$Y_2015)),
                            NYC_Map_Permits$ZipCode, NYC_Map_Permits$Block, NYC_Map_Permits$Lot, NYC_Map_Permits$Address) %>%
  lapply(htmltools::HTML)
...
"&copy; <a href=\"http://cartodb.com/attributions\">CartoDB</a>"
  ...
  #2001-2005 Layer:
  addPolygons(data = NYC_Map_Permits,group="2001-2005",
              color = "#444444",
              weight = 1,
              #smoothFactor = 0.2,
              opacity = 1.0,
  ...
  #2001-2010 Layer:
  addPolygons(data = NYC_Map_Permits,group="2001-2010",
              color = "#444444",
              weight = 1,
              #smoothFactor = 0.2,
              opacity = 1.0,
  ...
  #2001-2015 Layer:
  addPolygons(data = NYC_Map_Permits,group="2001-2015",
              color = "#444444",
              weight = 1,
              #smoothFactor = 0.2,
              opacity = 1.0,
...
# #Before merging, lets filter re_var index by year 2015 (so we get cummulative variation index): 
# permits_var <- filter(permits_var, Year == 2015)

#merge both datasets and replace NA's with zero:
NYC_Map_RE_Q <- merge(NYC_Map2, re_var_spread, by="Block")
NYC_Map_RE_Q@data[is.na(NYC_Map_RE_Q@data)] <- 0
NYC_Map_RE_Q@data$avg.index[NYC_Map_RE_Q@data$Distance=="other"] <- 0

#set color scale to be used when mapping variation index for RE sales:
palette <- colorNumeric(palette = "Blues", domain = NYC_Map_RE_Q$Qty_Y_2017) #continuous range

#Set interactive labels for 2014-2015 Layer:
block_labels2015 <- sprintf("<strong>%s from 2nd Ave Line:</strong> %s avg. increase<br/>in RE Sales (2014-2015)<br/>Zip Code: %s, Block: %s, Lot: %s<br/>Address: %s",NYC_Map_RE_Q$Distance,
                            ifelse(NYC_Map_RE_Q@data$Distance=='other',"NA", percent(NYC_Map_RE_Q$Qty_Y_2015)),
                            NYC_Map_RE_Q$ZipCode, NYC_Map_RE_Q$Block, NYC_Map_RE_Q$Lot, NYC_Map_RE_Q$Address) %>%
  lapply(htmltools::HTML)

#Set interactive labels for 2014-2016 Layer: 
block_labels2016 <- sprintf("<strong>%s from 2nd Ave Line:</strong> %s avg. increase<br/>in RE Sales (2014-2016)<br/>Zip Code: %s, Block: %s, Lot: %s<br/>Address: %s",NYC_Map_RE_Q$Distance,
                            ifelse(NYC_Map_RE_Q@data$Distance=='other',"NA", percent(NYC_Map_RE_Q$Qty_Y_2016)),
                            NYC_Map_RE_Q$ZipCode, NYC_Map_RE_Q$Block, NYC_Map_RE_Q$Lot, NYC_Map_RE_Q$Address) %>%
  lapply(htmltools::HTML)

#Set interactive labels for 2001-2015 Layer Shapefile: 
block_labels2017 <- sprintf("<strong>%s from 2nd Ave Line:</strong> %s avg. increase<br/>in RE Sales (2014-2017)<br/>Zip Code: %s, Block: %s, Lot: %s<br/>Address: %s",NYC_Map_RE_Q$Distance,
                            ifelse(NYC_Map_RE_Q@data$Distance=='other',"NA", percent(NYC_Map_RE_Q$Qty_Y_2017)),
                            NYC_Map_RE_Q$ZipCode, NYC_Map_RE_Q$Block, NYC_Map_RE_Q$Lot, NYC_Map_RE_Q$Address) %>%
  lapply(htmltools::HTML)
...
myNYCmap_re_q <- leaflet(options = leafletOptions(minZoom = 9, maxZoom = 18)) %>% #restringe/limit Zoom
  #addTiles() %>%  # Add default OpenStreetMap base map
...
"&copy; <a href=\"http://cartodb.com/attributions\">CartoDB</a>"
  ...
  #2014-2015 Layer:
  addPolygons(data = NYC_Map_RE_Q,group="2014-2015",
              color = "#444444",
              weight = 1,
              #smoothFactor = 0.2,
              opacity = 1.0,
              fillOpacity = 0.7,
              fillColor = ~palette(Qty_Y_2015),
              # change display properties while mouse cursor hovers on the polygon
              highlightOptions = highlightOptions(color="orange", 
                                                  weight=2, opacity=1, fillOpacity=1,
                                                  bringToFront = TRUE, sendToBack = TRUE),
  ...
  #2014-2016 Layer:
  addPolygons(data = NYC_Map_RE_Q,group="2014-2016",
              color = "#444444",
              weight = 1,
              #smoothFactor = 0.2,
              opacity = 1.0,
              fillOpacity = 0.7,
              fillColor = ~palette(Qty_Y_2016),
              # change display properties while mouse cursor hovers on the polygon
              highlightOptions = highlightOptions(color="orange", 
                                                  weight=2, opacity=1, fillOpacity=1,
                                                  bringToFront = TRUE, sendToBack = TRUE),
              label = block_labels2016,
              # label display options:
              labelOptions = labelOptions(style = list("font-weight" = "normal", 
                                          padding = "3px 8px"),
                                          textsize = "12px",
  ...
  #2014-2017 Layer:
  addPolygons(data = NYC_Map_RE_Q,group="2014-2017",
              color = "#444444",
              weight = 1,
              #smoothFactor = 0.2,
              opacity = 1.0,
              fillOpacity = 0.7,
              fillColor = ~palette(Qty_Y_2017),
              # change display properties while mouse cursor hovers on the polygon
              highlightOptions = highlightOptions(color="orange", 
                                                  weight=2, opacity=1, fillOpacity=1,
                                                  bringToFront = TRUE, sendToBack = TRUE),
  ...
  # legend 2014-2017
  leaflet::addLegend(pal = palette,
            values = NYC_Map_RE_Q$Qty_Y_2017,
            labFormat = labelFormat(prefix="",suffix="%",between=", ",transform=function(x) 100*x),
            group = "2014-2017",
            opacity = 0.7,
            title = "Avg. increase in RE Sales Qty",
            position = "bottomright") %>%
  ...
  # Layers control
  addLayersControl(
    baseGroups = c("2014-2015","2014-2016","2014-2017"),
    overlayGroups = c("See Subway Lines", "See Subway Stations"),
    position = "topleft",
    options = layersControlOptions(collapsed = FALSE))

myNYCmap_re_q  # Display the map

htmlwidgets::saveWidget(myNYCmap_re_q, 'Interactive_RE_Sales_Qty_Map-Html Widget.html', selfcontained = TRUE)
...
#merge both datasets and replace NA's with zero:
NYC_Map_RE_Notional <- merge(NYC_Map2, re_var_spread, by="Block")
NYC_Map_RE_Notional@data[is.na(NYC_Map_RE_Notional@data)] <- 0
NYC_Map_RE_Notional@data$avg.index[NYC_Map_RE_Notional@data$Distance=="other"] <- 0

#set color scale to be used when mapping variation index for RE sales:
palette <- colorNumeric(palette = "Blues", domain = NYC_Map_RE_Notional$Notional_Y_2017) #continuous range

#Set interactive labels for 2014-2015 Layer:
block_labels2015 <- sprintf("<strong>%s from 2nd Ave Line:</strong> %s avg. increase<br/>in RE Notional Sales (2014-2015)<br/>Zip Code: %s, Block: %s, Lot: %s<br/>Address: %s",NYC_Map_RE_Notional$Distance,
                            ifelse(NYC_Map_RE_Notional@data$Distance=='other',"NA", percent(NYC_Map_RE_Notional$Notional_Y_2015)),
                            NYC_Map_RE_Notional$ZipCode, NYC_Map_RE_Notional$Block, NYC_Map_RE_Notional$Lot, NYC_Map_RE_Notional$Address) %>%
  lapply(htmltools::HTML)

#Set interactive labels for 2014-2016 Layer: 
block_labels2016 <- sprintf("<strong>%s from 2nd Ave Line:</strong> %s avg. increase<br/>in RE Notional Sales (2014-2016)<br/>Zip Code: %s, Block: %s, Lot: %s<br/>Address: %s",NYC_Map_RE_Notional$Distance,
                            ifelse(NYC_Map_RE_Notional@data$Distance=='other',"NA", percent(NYC_Map_RE_Notional$Notional_Y_2016)),
                            NYC_Map_RE_Notional$ZipCode, NYC_Map_RE_Notional$Block, NYC_Map_RE_Notional$Lot, NYC_Map_RE_Notional$Address) %>%
  lapply(htmltools::HTML)

#Set interactive labels for 2001-2015 Layer Shapefile: 
block_labels2017 <- sprintf("<strong>%s from 2nd Ave Line:</strong> %s avg. increase<br/>in RE Notional Sales (2014-2017)<br/>Zip Code: %s, Block: %s, Lot: %s<br/>Address: %s",NYC_Map_RE_Notional$Distance,
                            ifelse(NYC_Map_RE_Notional@data$Distance=='other',"NA", percent(NYC_Map_RE_Notional$Notional_Y_2017)),
                            NYC_Map_RE_Notional$ZipCode, NYC_Map_RE_Notional$Block, NYC_Map_RE_Notional$Lot, NYC_Map_RE_Notional$Address) %>%
  lapply(htmltools::HTML)
...
myNYCmap_re_notional <- leaflet(options = leafletOptions(minZoom = 9, maxZoom = 18)) %>% #restringe/limit Zoom
  #addTiles() %>%  # Add default OpenStreetMap base map
...
"&copy; <a href=\"http://cartodb.com/attributions\">CartoDB</a>"
  ...
  #2014-2015 Layer:
  addPolygons(data = NYC_Map_RE_Notional,group="2014-2015",
              color = "#444444",
              weight = 1,
              #smoothFactor = 0.2,
              opacity = 1.0,
              fillOpacity = 0.7,
              fillColor = ~palette(Notional_Y_2015),
              # change display properties while mouse cursor hovers on the polygon
              highlightOptions = highlightOptions(color="orange", 
                                                  weight=2, opacity=1, fillOpacity=1,
                                                  bringToFront = TRUE, sendToBack = TRUE),
  ...
  #2014-2016 Layer:
  addPolygons(data = NYC_Map_RE_Notional,group="2014-2016",
              color = "#444444",
              weight = 1,
              #smoothFactor = 0.2,
              opacity = 1.0,
              fillOpacity = 0.7,
              fillColor = ~palette(Notional_Y_2016),
              # change display properties while mouse cursor hovers on the polygon
              highlightOptions = highlightOptions(color="orange", 
                                                  weight=2, opacity=1, fillOpacity=1,
                                                  bringToFront = TRUE, sendToBack = TRUE),
  ...
  #2014-2017 Layer:
  addPolygons(data = NYC_Map_RE_Notional,group="2014-2017",
              color = "#444444",
              weight = 1,
              #smoothFactor = 0.2,
              opacity = 1.0,
              fillOpacity = 0.7,
              fillColor = ~palette(Notional_Y_2017),
              # change display properties while mouse cursor hovers on the polygon
              highlightOptions = highlightOptions(color="orange", 
                                                  weight=2, opacity=1, fillOpacity=1,
                                                  bringToFront = TRUE, sendToBack = TRUE),
  ...
  # legend 2014-2017
  leaflet::addLegend(pal = palette,
            values = NYC_Map_RE_Notional$Notional_Y_2017,
            labFormat = labelFormat(prefix="",suffix="%",between=", ",transform=function(x) 100*x),
            group = "2014-2017",
            opacity = 0.7,
            title = "Avg. increase in RE Sales Notional Sales",
            position = "bottomright") %>%
...
myNYCmap_re_notional  # Display the map

htmlwidgets::saveWidget(myNYCmap_re_notional, 'Interactive_RE_Sales_Notional_Map-Html Widget.html', selfcontained = TRUE)
