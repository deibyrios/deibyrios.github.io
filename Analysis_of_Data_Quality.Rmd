...
Now we confirmed that there are no NA's in the data so the quality of the data is very good.
...
### NYC Dogs Licensing Data

In the Dogs Licensing Data set, we are provided with the dog registration information in all New York State from 2014 to 2016. This time period is conveniently coherent with our Real Estate Sales data and Building Permit Dataset, and reflected the trend of how dogs population changed as the Q Line – phase 1 of Second Avenue Subway was constructed.
...
library(mi)
show(missing_data.frame(dogs1))
show(missing_data.frame(dogs2))@patterns

```


After cleaning and filtering only the columns we wanted to use, we can see that our resulting data frame is fairly straight forward with most columbs without missing data. The only columns that has a lot of missing data is "breed", which is caused by the unknown breed of the dog or difficulty to put into a certain category due to the mixed breed. We kept these "uncertain" breeds as the count of the dogs are still important, while we left them out when we conducted the specific analysis later on. 

The key variables we kept for dogs analysis are: 

**Borough**: Owner Borough : indicates where this dog's owner resides 
**ZipCode**: Owner zip code : indicates the location of this dog and its owner
**breed**: Breed of the dog
**gender**: Gender of the dog
**LicenseIssuedDate**: Date the dog license was issued
**LicenseExpiredDate **: Date the dog license was expired

Now we confirmed that there are no NA's, and the missing data is not going to affact our analysis negatively, the quality of the data is very good.