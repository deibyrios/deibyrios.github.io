...
Since we are interested in the sale price, we also looked for zero values:
...
One of the analysis we decided to perform to answer our main question, was to see if the 2nd Ave line project has affected the real estate sale transactions, specifically the Sale Price, in the areas immediately in the vicinity of phase 1 and 2 of the 2 Ave Subway Line project (phase 3 and 4 have no funding commitments yet).
...
The graphs above doesn't allow us to make strong conclusions at first, be something that we immediately noted (as we are finance geeks) is that during the first year of the 2008 recession year, the sales price increased for properties within one block left to the line, while it decreased for the other categories, which may have been caused by the start of the construction of phased 1, during mid 2007 but then it started to decrease again, maybe due to due to news of financial problems with the project (whose construction had already been started and halted several times in the past).
...
We will now perform the analysis for buildings to the right of the Line and see if our theory above about during recession may become stronger or not.
<br>
...
From the graph above, as we had to some exempt anticipated, the 2nd Ave Line project seems to clearly affect properties located to the right, but not so much properties to the left, because the pre-existing Lexington-Fourth Avenue green Line is just 2 blocks left from the new 2nd Ave Line.
...
blocks_df_var <- blocks_df_var %>%
  group_by(Distance) %>%
  mutate(avg.index = Total/Total[1]) %>% ungroup()
...
library(ggthemes)
plot <- ggplot(data_phase1 , aes(Year, avg.index, color = Distance)) + geom_line(lwd = 1) +
    ggtitle("Market Value Vs Distance to 2nd Ave (Phase1)") +
    labs (x = "Year", y = "Indexed Variation in Market Value") +
    theme_grey(16) +
    theme(legend.title = element_blank()) +
    scale_colour_colorblind()
...
block_labels2007 <- sprintf("<strong>%s:</strong> %s avg. increase<br/>in Sales Prices (2005-2007)<br/>Zip Code: %s, Block: %s, Lot: %s<br/>Address: %s",NYC_Map2$Distance,
                            ifelse(NYC_Map2@data$Distance=='other',"NA", percent(NYC_Map2$Y_2007)),
                            NYC_Map2$ZipCode, NYC_Map2$Block, NYC_Map2$Lot, NYC_Map2$Address) %>%
  lapply(htmltools::HTML)

#Set interactive labels for 2005-2014 Layer: 
block_labels2014 <- sprintf("<strong>%s:</strong> %s avg. increase<br/>in Sales Prices (2005-2014)<br/>Zip Code: %s, Block: %s, Lot: %s<br/>Address: %s",NYC_Map2$Distance,
                            ifelse(NYC_Map2@data$Distance=='other',"NA", percent(NYC_Map2$Y_2014)),
                            NYC_Map2$ZipCode, NYC_Map2$Block, NYC_Map2$Lot, NYC_Map2$Address) %>%
  lapply(htmltools::HTML)

#Set interactive labels for 2005-2017 Layer Shapefile:
block_labels2017 <- sprintf("<strong>%s:</strong> %s avg. increase<br/>in Sales Prices (2005-2017)<br/>Zip Code: %s, Block: %s, Lot: %s<br/>Address: %s",NYC_Map2$Distance,
                            ifelse(NYC_Map2@data$Distance=='other',"NA", percent(NYC_Map2$Y_2017)),
                            NYC_Map2$ZipCode, NYC_Map2$Block, NYC_Map2$Lot, NYC_Map2$Address) %>%
  lapply(htmltools::HTML)
...
"&copy; <a href=\"http://cartodb.com/attributions\">CartoDB</a>"
  ...
  leaflet::addLegend(pal = palette,
            ...
            opacity = 0.7,
            title = "Avg. increase in R.E. Sales price",
